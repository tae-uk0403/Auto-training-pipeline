...
def calc_dists(preds, target, normalize):
    ...
    real_dists = np.zeros((preds.shape[1], preds.shape[0]))
    for n in range(preds.shape[0]):
        for c in range(preds.shape[1]):
            if target[n, c, 0] > 1 and target[n, c, 1] > 1:
                ...
                real_dists[c, n] = np.linalg.norm(preds[n, c, :] - target[n, c, :])
            else:
                dists[c, n] = -1
    return dists, real_dists
...
def accuracy(type, output, target, target_type='gaussian', thr=0.5, h=96, w=72):
    '''
    ...
    dists, real_dists = calc_dists(pred, target, norm)
    if type == "valid":
        for i in range(2):
            print("distance is : ",real_dists[i])
